...
var fs             = require('fs');
var DsSrvPreRenderError = exports.DsSrvPreRenderError = require('../error').DsSrvPreRenderError;
...
var dataTree = exports.dataTree = function (filename) {
  ...
  try{
    ...
    e.filename  = filename;
    e.stack     = null;
    throw new DsSrvPreRenderError(e);
  }
...
var outputPath = exports.outputPath = function(source, allowAlternateExtensions){
  ...
  for(var targetExtension in processors){ // .html, .css, .js
    if (processors.hasOwnProperty(targetExtension)) {
      processors[targetExtension].forEach(function(sourceExtension){ // .jade, .ejs, .md .pug
...
exports.shouldIgnore = function(filePath){
  ...
  /*
  if (filePath.indexOf('_') == 1 ) return res.end('404')
  if (filePath.indexOf('.') == 1 ) return res.end('404')

  if (filePath.indexOf('.php') > -1 ) return res.end('404')
  if (filePath.indexOf('_views') > -1 ) return res.end('404')

  if (filePath.indexOf('.pug') > -1 ) return res.end('404')
  if (filePath.indexOf('.jade') > -1 ) return res.end('404')
  if (filePath.indexOf('.ejs') > -1 ) return res.end('404')
  */
  // test for starting underscore, .,.git, .gitignore
  var map = arr.map(function(item){
    return item[0] === '_' || item.indexOf('.') === 0 || filePath.indexOf('.php') > -1;
  });
